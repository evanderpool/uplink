...
**Add sources** opens a dialog rather than a cramped side panel. Drop or pick as many files as you like and each is analysed before anything is sent — type, size, and whether it can be indexed at all — with a summary of what the batch amounts to. Choose an existing collection or create a new one by naming it. A **Paste text** tab takes typed or pasted prose directly, saving it as a Markdown source so a note is chunked, cited and verifiable like any file.

Files upload one at a time on purpose: every upload re-indexes and takes the database write lock, so parallel uploads would only contend. A file that fails does not stop the batch, and pressing the button again retries only the failures.
...
The suite (322 tests) covers every extractor (including a byte-level
